...
class EventAccessorInjectorScript(InjectorScriptBase):
    ...
    def create_script_file(self):
        ...
        super().priv_create_script_file("#!/bin/bash\n"
            ...
            "DBUS_CALL_ARGS=\"$SERVICE_GPUMGR /xyz/openbmc_project/GpuMgr xyz.openbmc_project.GpuMgr.Server DeviceGetData isi\"\n"
            "\n"
            "## global variables\n"
            "gl_property_value=\"\"\n"
            "gl_property_type=\"\"\n"
            ...
            "   echo; echo running in DRY-RUN mode...; echo ... ; sleep 2\n"
            "fi\n"
            "\n"
            "createMapKey()\n"
            "{\n"
            ...
            "dbus_call()\n"
            "{\n"
            "    local property_index=$(echo $METHOD_VALUE | awk -F : '{print $1}')\n"
            "    local property_name=$(echo $METHOD_VALUE | awk -F : '{print $NF}')\n"
            "    local cmd=\"busctl call $DBUS_CALL_ARGS $property_index $property_name 1\"\n"
            "    echo \"dbus_call(): $cmd\"\n"
            "    local result=$(eval \"$cmd\" 2> $gl_error_file)\n"
            "    gl_rc=$?\n"
            "    gl_dbus_call_result=\"\"\n"
            "    if [ $gl_rc -eq 0 ]; then\n"
            "       # result should like as: (isau) 0 'Baseboard GPU over temperature info : 0001' 2 1 0\n"
            "       echo \" $result\"\n"
            "       local vtemp=$(echo $result | awk -F : '{print $2}' | tr -d \\\' | tr -d \\\" | awk  '{print $1}' 2>$gl_error_file)\n"
            "       local counter=0\n"
            "       while [ \"${vtemp:$counter:1}\" = \"0\" ]; do\n"
            "           counter=$[ $counter + 1 ]\n"
            "       done\n"
            "       gl_dbus_call_result=$(printf \"0x%x\" ${vtemp:$counter})\n"
            "       echo \" using value: '$gl_dbus_call_result'\"\n"
            "    fi\n"
            ...
            "    local result=0\n"
            "    case \"$METHOD\" in \n"
            "       \"bitmask_dbus_call\") dbus_call\n"
            "                              if [ \"$gl_dbus_call_result\" != \"\" ]; then\n"
            "                                 export METHOD_VALUE=$gl_dbus_call_result\n"
            "                                 export METHOD=\"bitmask\"\n"
            "                                 change_value_method $1 # call itself\n"
            "                              fi;;\n"
            ...
            "           then\n"
            "                gl_old_property_value=$gl_new_property_value\n"
            "                property_set \"$@\"\n"
            "           fi\n"
            "        fi\n"
            "    fi\n"
            ...
            "    fi\n"
            "\n"
            "    cmd=\"busctl set-property `getService $1`  $@ $gl_property_type"
            " \'$gl_new_property_value\'\"\n"
            ...
            "        fi\n"
            "    fi\n"
            "    event=$(busctl get-property xyz.openbmc_project.Logging /xyz/openbmc_project/logging/entry/$1 xyz.openbmc_project.Logging.Entry Message | cut -f2-90 -d' ' | tr -d \\\")\n"
            "    if [ \"$event\" = \"$EVENT\" ]; then\n"
            "        device=$(busctl get-property xyz.openbmc_project.Logging /xyz/openbmc_project/logging/entry/$1 xyz.openbmc_project.Logging.Entry AdditionalData  \\\n"
            ...
            "        echo \"\t[$gl_counter_commands/$gl_total_comands] DEVICE='$DEVICE' EVENT='$EVENT'\" >> $FAILURES_FILE\n"
            "    else\n"
            "        gl_injections=$[ $gl_injections + 1 ]\n"
            "    fi\n"
            ...
            "       echo \" DEVICE='$DEVICE' EVENT='$EVENT' ACCESSOR_TYPE='$ACCESSOR_TYPE'\"\n"
            "    else\n"
            "       echo ============\n"
            "    fi\n"
            "\n"
            ...
            "       do\n"
            "          echo -ne \"[debug: $gl_counter_commands/$gl_total_comands] already inserted: $*, looking back at entry $gl_current_logging_entries ...\r\"\n"
            ...
            "fi\n"
            "echo; echo existent log entries = $gl_previous_logging_entries\n"
            "\n"
            "\n")
    ...
    def __store_command_information(self, device_item, method, method_value, accessor_type):
        """
    ...
    def __write_busctl_commands_into_script(self, cmd_info):
        ...
        if len(self._busctl_info) > com.INDEX_EVENT:
            cmd += f" DEVICE=\"{cmd_info['device']}\" EVENT=\"{cmd_info['event']}\" "
        cmd += f"property_change {cmd_info['device_item']} "
    ...
    def generate_script_from_json(self):
        ...
        if total_commands > 0:
            ...
            for cmd_info in self._commands_information_list:
                ...
                if total_commands == 0:
                    break
            bottom = "\n\necho; echo; echo \"Failed Injections: $gl_failures\"\ncat $FAILURES_FILE\n\n/bin/rm -rf $MAPFILE $FAILURES_FILE\n\necho; echo \"Successful Injections: $gl_injections\"\n"
